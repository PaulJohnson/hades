{-
Copyright © Paul Johnson 2019. See LICENSE file for details.
-}

-- |
--
-- Links from @gtk2hs@ to @reactive-banana@. Enables reactive programming with a GTK front end.

module Reactive.Banana.GI (
   module Reactive.Banana.GI.Common,
   module Reactive.Banana.GI.Connect,
   module Reactive.Banana.GI.Dialog,
   module Reactive.Banana.GI.Menu,
   module Reactive.Banana.GI.Table
) where

import Reactive.Banana.GI.Common
import Reactive.Banana.GI.Connect
import Reactive.Banana.GI.Dialog
import Reactive.Banana.GI.Menu
import Reactive.Banana.GI.Table
